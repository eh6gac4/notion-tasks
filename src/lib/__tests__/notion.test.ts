...
describe("updateTaskBlocks", () => {
  ...
  it("差分更新: delete を挟んだ insert は 1 つの append にまとめる (順序逆転防止)", async () => {
    ...
    const allAppendedTexts = appendCalls.flatMap(
      (c: Array<{ children: Array<{ paragraph?: { rich_text: Array<{ text: { content: string } }> } }> }>) =>
        c[0].children.map((b: { paragraph?: { rich_text: Array<{ text: { content: string } }> } }) => b.paragraph?.rich_text[0].text.content)
